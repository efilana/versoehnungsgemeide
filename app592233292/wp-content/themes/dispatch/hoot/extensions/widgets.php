		...
		?><div class="hoot-widget-field hoot-widget-field-type-<?php echo ( strlen( $type ) < 15 ) ? sanitize_html_class( $type ) : 'custom' ?> hoot-widget-field-<?php echo sanitize_html_class( $id ) ?>"><?php
			...
			switch( $type ) {
				...
				case 'color':
					...
					?><input id="<?php echo $this->hoot_get_field_id( $id, $repeater ) ?>" class="hoot-of-color" name="<?php echo $this->hoot_get_field_name( $id, $repeater ) ?>" type="hidden" value="<?php echo esc_attr( $value ) ?>" <?php echo $default_color; ?> />
					<?php
					break;
				...
				case 'group':
					...
					?><div class="hoot-widget-field-group" data-id="<?php echo esc_attr( $id ) ?>">
						...
						?>
						<div id="add-<?php echo rand(1000, 9999); ?>" class="hoot-widget-field-group-add" data-iterator="<?php echo is_array( $value ) ? max( array_keys( $value ) ) : 0; ?>"><?php _e('Add', 'dispatch') ?></div>
					</div>
					<?php
					break;
	...
	function render_group( $key, $value, $fields, $item_name = '', $repeater = array() ){
		...
		<div class="hoot-widget-field-group-item">
			<div class="hoot-widget-field-group-item-top">
				...
				<h4><i class="fas fa-caret-down"></i> <?php echo esc_html( $item_name ) ?></h4>
			</div>
	...
	static function get_pages( $number, $post_type = 'page' ){
		...
		if ( !empty( $the_query->posts ) )
			foreach ( $the_query->posts as $post ) if( !empty( $post->ID ) )
				$options_pages[ $post->ID ] = ( empty( $post->post_title ) ) ? '' : apply_filters( 'the_title', $post->post_title );
		return $options_pages;
	}
